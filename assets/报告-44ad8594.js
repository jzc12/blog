...
</code></pre><ul><li><code class="">transition_dict</code> 是一个包含以下键的字典： <ul><li><code class="">&#39;rewards&#39;</code>：智能体在每一步获得的即时奖励 <code class="">r_t</code></li><li><code class="">&#39;states&#39;</code>：每一步对应的状态 <code class="">s_t</code></li><li><code class="">&#39;actions&#39;</code>：每一步智能体选择的动作 <code class="">a_t</code></li></ul></li></ul><p>这些是一次完整回合（episode）的轨迹数据。</p><hr><h3>二、计算 reward-to-go（回报）</h3><pre><code class="language-python">G_list = []
...
for i in range(len(rewards)):
    ...
    for j in range(i, len(rewards)):
        ...
        discount *= self.gamma
    G_list.append(G)
</code></pre><p>这部分是计算每个时间步的 累积回报（Reward-to-Go）：</p><ul><li>$G_t = r_t + γ·r_{t+1} + γ²·r_{t+2} + …$</li><li></li><li>G_list[i] 存的是从第 <code class="">i</code> 步开始累加到终点的折扣回报。</li></ul><p>这一步的作用是为每个动作赋予一个“价值”，用来衡量其“好坏”。</p><hr><h3>三、奖励标准化（稳定训练）</h3><pre><code class="language-python">G_tensor = torch.tensor(G_list, dtype=torch.float).to(self.device)
G_tensor = (G_tensor - G_tensor.mean()) / (G_tensor.std() + 1e-8)
</code></pre><p>将 <code class="">G_list</code> 转为 tensor，并做了标准化处理（均值为 0，标准差为 1）：</p><ul><li>目的：避免因为奖励值范围大、变化剧烈，导致网络更新不稳定。</li><li>1e-8 是为了防止除以 0。</li></ul><hr><h3>四、计算策略损失并反向传播</h3><pre><code class="language-python">for i in range(len(states)):
    ...
    loss = -log_prob * G_tensor[i]
</code></pre><ul><li>对每一个状态： <ul><li>计算当前策略对 <code class="">action[i]</code> 的概率</li><li>取对数概率 <code class="">log_prob</code></li><li>损失函数：<code class="">-log_prob * G_tensor[i]</code></li></ul></li></ul><blockquote><p><code class="">∇J(θ) ≈ ∇ log π_θ(a|s) * G_t</code></p></blockquote><hr><h2>A2C强化学习模型实现报告</h2><h3>选题意义</h3><table><thead><tr><th>内容项</th><th>说明</th></tr></thead><tbody><tr><td>项目背景</td><td>在策略梯度方法中，REINFORCE 存在高方差问题，难以稳定学习。Actor-Critic 方法通过引入“值函数”显著降低方差。</td></tr><tr><td>研究动机</td><td>采用 A2C 方法，可以结合策略网络和价值估计器，提升训练效率与稳定性，是通往 PPO、TD3 等主流算法的基础。</td></tr><tr><td>实用价值</td><td>适用于教学实验、控制类任务、OpenAI Gym 训练任务，如 CartPole、LunarLander 等。</td></tr><tr><td>课程关联</td><td>对应机器学习课程中的策略优化部分，是理解深度强化学习主流范式（Policy Gradient + Baseline）的重要案例。</td></tr></tbody></table><hr><h3>技术细节</h3><h4>模型结构（ACnet）：</h4><pre><code class="language-python"># 状态 -&gt; 特征
self.affine = nn.Linear(state_dim, hidden_dim)
# 特征 -&gt; 策略概率
self.action_layer = nn.Linear(hidden_dim, action_dim)
# 特征 -&gt; 状态值估计
self.value_layer = nn.Linear(hidden_dim, 1)
</code></pre><ul><li>使用共享特征层（<code class="">affine</code>），分别输出动作概率和状态值；</li><li>softmax 用于生成 <code class="">Categorical</code> 分布实现动作采样。</li></ul><h4>损失函数计算：</h4><pre><code class="language-python">advantages = discounted_rewards - state_values.detach()
policy_loss = -(logprobs * advantages).mean()
value_loss = F.smooth_l1_loss(state_values, discounted_rewards)
</code></pre><ul><li><code class="">advantages</code> 是回报减去 value baseline；</li><li>策略损失基于 Advantage，值函数使用 Huber Loss（更稳）；</li><li>最终损失为 <code class="">policy_loss + 0.5 * value_loss</code>。</li></ul><h4>更新逻辑（训练）：</h4><pre><code class="language-python">if episode % 20 == 0:
    loss = self.actor_critic.calculateLoss(self.gamma)
    loss.backward()
    torch.nn.utils.clip_grad_norm_(...)  # 梯度裁剪
</code></pre><ul><li>每 20 个 episode 更新一次（较保守）；</li><li>使用梯度裁剪避免爆炸；</li><li>使用 <code class="">orthogonal_</code> 初始化提升学习表现。</li></ul><hr><h3>创新点</h3><table><thead><tr><th>项目亮点</th><th>描述</th></tr></thead><tbody><tr><td>Advantage 用于策略更新</td><td>相比 REINFORCE 直接乘 Gt，这里用了 <code class="">A = G - V</code>，方差显著下降，训练更稳</td></tr><tr><td>共享特征提取层</td><td>减少冗余计算，结构紧凑</td></tr><tr><td>梯度裁剪与初始化</td><td>使用梯度裁剪 + 正交初始化，能有效防止 early divergence</td></tr><tr><td>模块化结构清晰</td><td>ACnet + ActorCritic 分层明确，后续可扩展为 PPO、GAE 等复杂结构</td></tr></tbody></table><hr><h3>问题反思</h3><table><thead><tr><th>问题</th><th>分析</th></tr></thead><tbody><tr><td>更新频率过低</td><td>每 20 个 episode 才更新一次，可能导致学习效率低下（可调小为每 5 或 10）</td></tr><tr><td>GPU 利用率低</td><td>无并行环境采样，不能发挥 GPU 并行优势</td></tr><tr><td>动作空间受限</td><td>只适用于离散动作（<code class="">Categorical</code>），若环境为连续动作需改为 <code class="">Normal</code> 分布</td></tr><tr><td>无熵正则</td><td>没有鼓励探索项（如 <code class="">entropy bonus</code>），容易陷入确定性策略</td></tr><tr><td>优化器参数固定</td><td><code class="">lr=0.02</code> 偏大，可能不适用于复杂环境（建议支持外部传参）</td></tr></tbody></table><ul><li>GAE（广义优势估计）</li><li>多线程/异步更新（A3C）</li><li>动作熵正则项</li><li>动态学习率调整</li></ul><pre><code class="language-text">Episode 100	Average Score: -182.44	
...
</code></pre><h3>3. <code class="">forward(self, state)</code></h3><p>算法流程：</p><ol><li>输入处理： <ul><li>将输入 <code class="">state</code> 转换为浮点张量并移至</li></ul></li><li>特征提取： <ul><li>通过 <code class="">affine</code> 层 + ReLU 激活，得到隐藏特征 <code class="">features</code>。</li></ul></li><li>Critic（价值头）： <ul><li><code class="">value_layer</code> 输出状态价值 <code class="">state_value</code></li></ul></li><li>Actor（动作头）： <ul><li><code class="">action_layer</code> 输出动作 logits → Softmax 归一化为概率 <code class="">action_probs</code>。</li><li>创建分类分布 <code class="">Categorical</code>，采样动作 <code class="">action</code>。</li></ul></li><li>轨迹记录： <ul><li>存储 <code class="">log_prob(action)</code>（策略梯度需用）和 <code class="">state_value</code>（后续计算优势函数）。</li></ul></li><li>返回动作： <ul><li>返回动作的标量值（<code class="">.item()</code> 脱离子图）。</li></ul></li></ol><hr><h3>4. <code class="">calculateLoss(self, gamma)</code></h3><p>功能：计算策略损失（Actor）和价值损失（Critic）。 算法流程：</p><ol><li><p>数据准备：</p><ul><li>将缓冲区中的 <code class="">rewards</code>、<code class="">logprobs</code>、<code class="">state_values</code> 转为张量。</li><li><code class="">state_values</code> 去掉多余的维度（<code class="">squeeze</code>）。</li></ul></li><li><p>计算折扣回报：</p><ul><li><p>逆序遍历 <code class="">rewards</code>，按公式计算累积折扣回报：</p></li><li><p>$G_t = r_t + \\gamma \\cdot G_{t+1} $</p></li><li><p>结果存入 <code class="">discounted_rewards</code>。</p></li></ul></li><li><p>回报标准化：</p><ul><li>减去均值，除以标准差稳定训练。</li></ul></li><li><p>优势函数计算：</p><ul><li>$\\text{advantages} = \\text{discounted_rewards} - \\text{state_values}.detach() $ （<code class="">detach()</code> 阻止梯度流向 Critic，仅用于评估策略）。</li></ul></li><li><p>损失计算：</p><ul><li>策略损失（Actor）： $ -\\mathbb{E}[\\log \\pi(a|s) \\cdot \\text{advantages}]$</li><li>价值损失（Critic）： 平滑 L1 损失（Huber 损失），减少 Critic 的估值误差。</li></ul></li><li><p>总损失：</p><ul><li>返回加权和：<code class="">policy_loss + 0.5 * value_loss</code></li></ul></li><li><p>Actor-Critic 协同：</p><ul><li>Actor：通过 <code class="">logprobs</code> 和优势函数更新策略，引导动作选择。</li><li>Critic：通过 <code class="">state_values</code> 和折扣回报的差异优化价值估计。</li></ul></li><li><p>折扣回报标准化：</p><ul><li>减少不同回合回报的尺度差异，加速收敛。</li></ul></li><li><p>优势函数分离梯度：</p><ul><li><code class="">state_values.detach()</code> 确保 Critic 的梯度不影响 Actor 的更新。</li></ul></li><li><p>正交初始化：</p><ul><li>保持前向传播中信号范数稳定，缓解梯度消失/爆炸。</li></ul></li></ol><table><thead><tr><th>目标内容</th><th>实现情况说明</th></tr></thead><tbody><tr><td>实现策略梯度算法</td><td>使用 REINFORCE 方法，含奖励标准化与 Monte Carlo 回报估计</td></tr><tr><td>实现 Actor-Critic 算法</td><td>包含 actor-critic 联合网络，使用 critic 进行优势估计</td></tr><tr><td>支持轨迹存储与损失计算</td><td>实现 log-prob、状态价值、奖励的存储与处理</td></tr><tr><td>奖励折扣与标准化处理</td><td>使用 gamma=0.99 计算折扣回报并进行标准化</td></tr></tbody></table><h3><strong>2. Actor-Critic 算法（1页PPT）</strong></h3><h4><strong>核心思想</strong></h4><p>联合训练策略网络（Actor）和价值网络（Critic），用Critic的 (V(s)) 减小方差。</p><hr><h4><strong>关键公式</strong></h4><p>优势函数：</p><p>$A(s,a) = R_t - V_\\phi(s)$</p><p>联合损失：</p><p>$\\mathcal{L} = -\\log \\pi_\\theta(a|s) \\cdot A(s,a) + 0.5 \\cdot (V_\\phi(s) - R_t)^2$</p><p>其中：</p><ul><li>$\\pi_\\theta$是Actor网络，$V_\\phi$是Critic网络</li><li>$R_t$</li></ul><pre><code class="language-mermaid">flowchart TD
    A[state] --&gt; B[共享层 state_dim --&gt; hidden_dim]
    B --&gt; C[action \\nhidden_dim --&gt; action_dim]
    B --&gt; D[critic\\nhidden_dim --&gt; 1]

</code></pre>`,90)])))}});export{i as default,h as excerpt,g as frontmatter};
