...
+----+-----+------+
   ...
   └──────┬────────┘
    ...
    └─────────────┘
</code></pre><p>CDN + 负载均衡（LB）：分散用户请求压力，缓存静态资源。</p><p>API 网关：做限流和鉴权，保护后端服务。</p><p>微服务层：</p><ul><li>票务服务：用分布式锁保证票数准确，利用缓存优化查询。</li><li>订单服务：通过数据库事务和消息队列处理订单，防止超卖。</li><li>用户服务：用户认证和管理。</li></ul><p>分布式数据库：水平拆分，提升并发处理能力。</p><p>缓存层：Redis缓存热点数据，快速响应。</p><p>消息队列：异步处理订单请求，削峰填谷。</p><h1>项目</h1><h2>编译器</h2><h3>1. 基础性</h3><ul><li>你实现的编译器支持哪些语法？比如是否支持嵌套语句、函数、数组等？</li><li>语法规则是你自己写的吗？有没有遇到 shift/reduce 冲突？怎么解决的？</li><li>你实现了哪些语义检查？比如类型检查、作用域检查有没有做？</li><li>Flex 中你定义的词法规则长什么样？举个例子。</li><li>中间代码是什么形式？三地址码、抽象语法树，还是 LLVM IR？</li><li>目标代码是你自己定义的汇编格式吗？能举例一段你生成的汇编代码吗？</li></ul><hr><h3>RISC-V 整数寄存器一览表</h3><table><thead><tr><th>寄存器</th><th>ABI 名称</th><th>用途</th><th>保存责任</th></tr></thead><tbody><tr><td>x1</td><td>ra</td><td>返回地址</td><td>Caller</td></tr><tr><td>x2</td><td>sp</td><td>栈指针</td><td>Calle</td></tr><tr><td>x10-11</td><td>a0-a1</td><td>函数参数/返回值</td><td>Caller</td></tr><tr><td>x12-17</td><td>a2-a7</td><td>函数参数</td><td>Caller</td></tr><tr><td>x18-27</td><td>s2-s11</td><td>保存寄存器</td><td>Callee</td></tr><tr><td>x28-31</td><td>t3-t6</td><td>临时寄存器</td><td>Caller</td></tr></tbody></table><hr><h2>桌面精灵</h2><h3>1. 基础</h3><ul><li>你是怎么实现窗口透明的？怎么做到不抢焦点但又能响应点击？</li><li>你用 Hook 拦截了哪些系统消息？鼠标？窗口？怎么实现的？</li><li>你使用 spine-unity 加载动画时，是否遇到过资源加载延迟的问题？怎么处理？</li><li>为什么需要协程？协程在 Unity 和线程的区别是什么？</li><li>如果我希望这个精灵能“回应用户说话”（比如通过语音合成），你准备怎么集成这个功能？</li></ul><h2>QT</h2><h3>1.基础</h3><ul><li>Qt 不太适合做游戏，为什么选择它？用了哪些 Qt 模块？</li><li>你的游戏是怎么处理界面更新和逻辑计算的？用定时器？还是事件驱动？</li><li>有没有设计“资源流转”的机制？比如原材料→加工→运输→成品？</li><li>你的类结构设计是怎样的？比如“建筑”、“物品”、“生产线”是怎么封装的？</li><li>存档功能怎么实现的？是序列化对象？用 JSON 还是其他格式？</li></ul><h1>自我介绍</h1><h2>HR</h2><blockquote><p>面试官你好，我是 XXX，目前是南京大学智能软件与工程学院的大三学生。</p><p>我将从技术背景、校园经历，个人优势三个方面来自我介绍。</p><p>技术背景方面：</p><p>我的主要开发语言是 C++ 和 Python，也有使用过 Java 进行 UML 工具的课程项目开发。这学期一直保持力扣刷题，目前刷了 300 题。我学习的课程包括：操作系统、编译原理、软件系统设计、自动化测试、软件质量管理等。</p><p>校园经历方面</p><p>大一时加入了校内的南播玩社团，参与策划和组织过“绿色生活打卡周”“益童筑梦”等活动，也有负责过活动推文的撰写与总结报告发布。参与过 RAG 检索问答系统的团队开发项目，完成了后端评估和登录系统部分，积累了团队协作和项目管理的实际经验。</p><p>个人优势方面</p><p>我善于学习并应用新的知识。</p><p>比如自学 Unity、C#、游戏资源解包 等内容，从零完成了一个桌面精灵交互系统：包括 AB 包解包、Spine 动画接入、状态切换、协程管理等功能，也写了自动化脚本来优化资源处理流程。</p><p>我也在尝试在 GitHub 上创建博客，整理学习内容和项目经验，提升表达能力（现在进度还没有多少）。</p><p>我能够很明显的感受到，进入大学的这几年我一直在成长，就比如去年在写一个登录页面的前后端实现的时候，总是有BUG，总是对接不上，然后其他事情的原因就放弃了，然后这学期，又尝试了一下，借助大模型，半小时就初步搭建了包括内网穿透、数据库搭建在网上的登录系统。</p><p>我希望能在贵公司实习，进一步提升自己的能力。</p><p>谢谢！</p></blockquote><h2>技术</h2><p>面试官你好，我是 XXX，目前是南京大学智能软件与工程学院的大三学生。</p><p>我将从技术背景、代码规范，个人优势三个方面来自我介绍。</p><p>技术背景方面：</p><p>我的主要开发语言是 C++ 和 Python，也有使用过 Java 进行 UML 工具的课程项目开发。这学期一直保持力扣刷题，目前刷了 300 题。我学习的课程包括：操作系统、编译原理、软件系统设计、自动化测试、软件质量管理等。</p><p>代码规范方面</p><p>个人优势方面</p><p>我善于学习并应用新的知识。</p><p>比如我曾经自学 Unity、C#、游戏资源解包 等内容，从零完成了一个桌面精灵交互系统：包括 AB 包解包、Spine 动画接入、状态切换、协程管理等功能，写了自动化脚本来优化资源处理流程。</p><p>我也在尝试在 GitHub 上创建博客，整理学习内容和项目经验，提升表达能力与技术沉淀（现在进度还没有多少）。</p><p>我能够很明显的感受到，进入大学的这几年我一直在成长，就比如去年在写一个登录页面的前后端实现的时候，总是有BUG，总是对接不上，然后其他事情的原因就放弃了，但是这学期，偶然间又想尝试了一下，借助大模型，半小时就初步搭建了包括内网穿透、数据库搭建在网上的登录系统。</p><p>我希望能在贵公司实习，进一步提升自己的能力。</p><p>谢谢！</p><h1>面经</h1><p>时间：30min</p><ol><li>介绍项目？背景？时间？分工？困难点？成果？</li><li>项目的优点和缺点？</li><li>合作冲突？</li><li>如何了解和学习新知识？</li><li>项目承担角色？</li><li>项目中有和客户进行对接吗？</li><li>何时感受到荣耀感？</li></ol><p>1、学生表student。字段是name、age、class，统计每个班有多少人，并且按照每个班人数多少降序排列。 2、gdb调试 基本命令 怎么查看线程的工作状态。 3、linux系统的常用命令：（cd啥的就不用说了）。 4、java的自动装配原理。 5、数据库连接池的作用，数据库的脏读。 6、epoll等几种io复用的区别，epoll是怎么实现线程安全的，线程池是怎么实现的。 7、linux top命令是干什么的。 8、c++的智能指针，weak_ptr是怎么用的。</p><p>3.C++中多线程、多进程的通信 4.shared_ptr内部实现的机制 5.shared_ptr计数变量存放位置</p><ol><li>八股文类问题（高频）</li></ol><ul><li>内存管理：C内存分区（栈区寻址方式）、<code>resize</code>与<code>reserve</code>区别、<code>vector</code>与<code>list</code>底层实现。</li><li>面向对象：多态实现（虚函数机制、虚表指针）、纯虚函数与虚函数区别、基类析构函数为何需声明为虚函数。</li><li>语言特性：引用与指针的区别、<code>const</code>在拷贝构造函数中的作用、C++11新特性（<code>auto</code>、<code>lambda</code>、智能指针）。</li><li>智能指针：<code>shared_ptr</code>、<code>unique_ptr</code>、<code>weak_ptr</code>原理及使用场景。</li><li>并发与通信：进程间通信方式、线程同步方法。</li></ul><ol start="2"><li>网络与操作系统</li></ol><ul><li>网络协议：TCP与UDP区别、HTTP与HTTPS区别、TCP报文结构及数据安全机制。</li><li>操作系统：32位与64位机器判断方法、<code>xv6</code>操作系统功能、Shell命令自定义实现。</li></ul><ol start="3"><li>场景与设计</li></ol><ul><li>设计模式：单例模式的实现及线程安全问题。</li><li>调试能力：线上Bug定位方法（如日志分析、断点调试）。</li></ul><hr><h4>三、高频考点与趋势分析</h4><ol><li>核心能力考察： <ul><li>底层原理：虚函数机制、内存管理、智能指针等是必考内容。</li><li>实际应用：结合项目经验提问（如项目中的C++技术选型、性能优化）。</li><li>代码健壮性：如拷贝构造函数的<code>const</code>必要性、<code>vector</code>动态扩容机制。</li></ul></li><li>延伸技能需求： <ul><li>网络编程：TCP传输细节（如300MB文件传输时间估算）。</li><li>多线程开发：线程同步方式（互斥锁、信号量等）。</li></ul></li></ol><hr><h4>四、面试流程与建议</h4><ol><li>流程： <ul><li>技术面（1-2轮）：以八股文+项目深挖为主，部分涉及手撕代码。</li><li>HR面：职业规划、团队协作、抗压能力等软技能考察。</li></ul></li><li>建议： <ul><li>项目准备：重点梳理项目中C++相关模块，明确技术难点及解决方案。</li><li>八股强化：针对虚函数、内存管理、STL容器等高频问题系统复习。</li><li>模拟实战：通过<code>sizeof</code>与<code>strlen</code>区别等细节题查漏补缺。</li></ul></li></ol>`,304)])))}};export{u as category,c as date,b as default,s as summary,p as updated};
