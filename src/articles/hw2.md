...
设单位向量 $\mathbf{x} \in \mathbb{S}^{d-1}$，定义：$A_i$ 为 $A$ 的第 $i$ 行；$Y_i = \langle A_i, \mathbf{x} \rangle = \sum_{j=1}^d A_{ij} x_j$；
...
\Rightarrow \mathbb{E}[\|A \mathbf{x}\|_2^2] = \sum_{i=1}^k \mathbb{E}[Y_i^2] = 1
$
...
$
\text{Var}(Y_i) = \frac{1}{k} \sum_{j=1}^d x_j^2 \le \frac{1}{k} \cdot d \cdot \alpha^2
...
3.1  对于 metric space $([0,n]^d, d)$，距离定义为 $ d(x,y) = \sqrt{\displaystyle\sum_i \min(|x_i - y_i|, n - |x_i - y_i|)^2}$
即每一维上的距离是模 $n$ 的圆周距离。
...
使用 JL 定理可以将维度降至 $k=O(\epsilon^{-2}\log n)$ 以提高效率。
...
对每个向量 $x \in S^{d-1}$，随机选取 $b \sim \mathcal{N}(0,I)$，定义哈希函数：$h_b(x) = \text{sign}(\langle b, x \rangle) \in \{-1, +1\}$
对任意 $x, y \in S^{d-1}$，有：$\Pr[h_b(x) = h_b(y)] = 1 - \frac{\theta(x,y)}{\pi}, \quad \theta(x,y) = \arccos(\langle x, y \rangle)$
表明夹角越小，碰撞概率越高。
...
若 $\sigma(k_i) = \sigma(k_j)$，则：$\prod A x = \prod (a_i - a_j) = \prod a_i - \prod a_j = e_{\sigma(k_i)} - e_{\sigma(k_j)} = 0, \quad \Rightarrow \|\prod A x\|_2^2 = 0.$



定义事件 $\mathcal{E}_{i,j}$ 为：$\mathcal{E}_{i,j} := \{k_i \ne k_j \text{ 且 } \sigma(k_i) = \sigma(k_j)\}$

有 $\|A x\|_2^2 = 2$，但 $\|\prod A x\|_2^2 = 0$，所以违反了 OSE 条件。
...
对于任意固定的 $\sigma$，令 $w_r = |\sigma^{-1}(r)|$ 表示第 $r$ 个桶中元素个数，则有：

$\displaystyle\sum_{r=1}^m w_r = n, \quad \sum_{r=1}^m w_r^2 \ge \frac{n^2}{m} \quad \text{(Cauchy-Schwarz 定理)}.$

$\therefore \mathbb{P}(\sigma(k_i) = \sigma(k_j) \mid k_i \ne k_j) = \frac{1}{n(n-1)} \sum_{r=1}^m w_r(w_r - 1) \ge \frac{1}{n^2} \sum_{r=1}^m w_r^2 \ge \frac{1}{m}.$

存在常数 $c > 0$ 使得：$\mathbb{P}(\mathcal{E}_{i,j}) \ge \frac{c}{m}$

记冲突对数为随机变量：

$X = \displaystyle\sum_{1 \le i < j \le d} \mathbf{1}_{\mathcal{E}_{i,j}}, \quad \mathbb{E}[X] \ge \binom{d}{2} \cdot \frac{c}{m} = \Omega\left(\frac{d^2}{m}\right).$
...
1. 对于每个节点 $i \in V$，在 $G'$ 中创建两个新节点：$i_{\text{in}}$ 和 $i_{\text{out}}$ ，添加一条从 $i_{\text{in}}$ 到 $i_{\text{out}}$ 的有向边，容量为 $c_i$；对于原始图中的每条边 $(u, v) \in E$， 在 $G'$ 中，添加一条从 $u_{\text{out}}$ 到 $v_{\text{in}}$ 的有向边，容量设置为一个足够大的数，代表原始边的无穷大容量。
    ...
    那么 $G'$ 的源点为 $s_{\text{in}}$  汇点为 $t_{\text{out}}$， 而且新的图节点数为 $2 \times |V|$， 边的数量为 $|V| + |E|$
...
复杂性分析：图的大小为 $O(|V| + |E|)$，算法复杂性与标准最大流算法相同， $O(|V|^2 |E|)$  或者  $O(|V| |E|^2)$
...
int min_cost(vector<int>& nums, vector<int>& cost, int demand) {
    ...
    for (int i = 0; i < n; i++) {
        int quantity = nums[i];
        int price = cost[i];
        // 枚举供应商 i 提供的 1 ~ quantity 个零件
        for (int q = 1; q <= quantity; ++q) {
            for (int v = MAX - 1; v >= q; --v) {
                dp[v] = min(dp[v], dp[v - q] + q * price);
            }
        }
    }
    // 统计满足需求量 ≥ demand 的最小成本
    int res = INF;
    for (int i = demand; i < MAX; ++i) {
        res = min(res, dp[i]);
    }
...
若 Primal 和 Dual 都有最优解，则最优值相等、Primal 有整系数、有限边界，因此满足强对偶定理条件。

因此：

$\text{最大匹配 LP 的最优值} = \text{最小顶点覆盖 LP 的最优值}$
